...
echo "\$(srcdir)/liquidwar6-gdoc.texi: "`ls ../src/lib/*.[ch] | sort` >> Makefile.gdoc
...
for i in bot cfg cli cns cnx dat dsp dyn gfx glb gui hlp img ker ldr map msg net nod p2p pil scm snd srv sys tsk vox ; do
    echo "\$(srcdir)/$i-gdoc.texi: "`ls ../src/lib/$i/*.[ch] | sort` >> Makefile.gdoc
...
for i in bot/mod-brute bot/mod-follow bot/mod-idiot bot/mod-random gfx/mod-gl snd/mod-csound snd/mod-ogg cli/mod-tcp cli/mod-udp cli/mod-http srv/mod-tcpd srv/mod-udpd srv/mod-httpd ; do
    j=`echo $i | cut -d "/" -f 2`
    echo "\$(srcdir)/$j-gdoc.texi: "`find ../src/lib/$i -name "*.[ch]" | sort` >> Makefile.gdoc
    echo "\techo gdoc $j && cd \$(top_srcdir)/doc && \$(PERL) \$(top_srcdir)/doc/gdoc.pl -texinfo \$^ > \$(top_srcdir)/doc/$j-gdoc.texi" >> Makefile.gdoc
    echo >> Makefile.gdoc
    perl gdoc.pl -texinfo `find ../src/lib/$i -name "*.[ch]" | sort` > $j-gdoc.texi
done
