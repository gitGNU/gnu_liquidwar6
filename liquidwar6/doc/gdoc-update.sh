...
echo "\$(top_srcdir)/doc/liquidwar6-gdoc.texi: "`ls ../src/lib/*.c* | sed "s/\.\./\$\(top_srcdir\)/g" | sort` >> Makefile.gdoc
echo "\techo gdoc $i && cd \$(top_srcdir)/doc && \$(PERL) gdoc.pl -texinfo \$^ > liquidwar6-gdoc.texi" >> Makefile.gdoc
echo >> Makefile.gdoc
...
for i in bot cfg cli cns cnx dat dsp dyn gen gfx glb gui hlp img ker ldr map msg net nod p2p pil scm sim snd srv sys tsk vox ; do
    ...
    echo "\techo gdoc $i && cd \$(top_srcdir)/doc && \$(PERL) gdoc.pl -texinfo \$^ > $i-gdoc.texi" >> Makefile.gdoc
    echo >> Makefile.gdoc
...
for i in bot/mod-brute bot/mod-follow bot/mod-idiot bot/mod-random gfx/mod-gl1 gfx/mod-gles2 gfx/mod-soft gfx/shared-sdl gfx/mod-caca snd/mod-csound snd/mod-ogg cli/mod-tcp cli/mod-udp cli/mod-http srv/mod-tcpd srv/mod-udpd srv/mod-httpd ; do
    ...
    echo "\techo gdoc $j && cd \$(top_srcdir)/doc && \$(PERL) gdoc.pl -texinfo \$^ > $j-gdoc.texi" >> Makefile.gdoc
    echo >> Makefile.gdoc
