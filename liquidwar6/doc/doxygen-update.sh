...
echo "\$(srcdir)/liquidwar6-doxygen.texi: "`ls ../src/lib/*.h* | sed "s/\.\./\$\(top_srcdir\)/g" | sort` >> Makefile.doxygen
...
for i in bot cfg cli cns cnx dat dsp dyn gen gfx glb gui hlp img ker ldr map msg net nod p2p pil scm sim snd srv sys tsk vox ; do
    echo "\$(srcdir)/$i-doxygen.texi: "`ls ../src/lib/$i/*.h* | sed "s/\.\./\$\(top_srcdir\)/g" | sort` >> Makefile.doxygen
