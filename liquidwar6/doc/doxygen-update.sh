...
echo "\tif test -w \$@ || test ! -f \$@ ; then echo doxygen liquidwar6 && echo > \$@; for l in `echo doxygen/xml/structlw6__* doxygen/xml/unionlw6__* | sort` ; do if test -f \"\$\$l\"; then xsltproc \$(srcdir)/doxygen/texinfo-struct.xsl \"\$\$l\" | grep -v \"<?xml\" >> \$@; fi; done; fi" >> Makefile.doxygen 
echo >> Makefile.doxygen
...
for i in bot cfg cli cns cnx dat dsp dyn gen gfx glb gui hlp img ker ldr map mat msg net nod p2p pil scm sim snd srv sys tsk vox ; do
    ...
    echo "\tif test -w \$@ || test ! -f \$@ ; then echo doxygen $i && echo > \$@; for l in `echo doxygen/xml/structlw6${i}__* doxygen/xml/unionlw6${i}__* | sort` ; do if test -f \"\$\$l\"; then xsltproc \$(srcdir)/doxygen/texinfo-struct.xsl \"\$\$l\" | grep -v \"<?xml\" >> \$@; fi; done; fi" >> Makefile.doxygen
    echo >> Makefile.doxygen
...
for i in bot/mod-brute bot/mod-follow bot/mod-idiot bot/mod-random gfx/mod-gl1 gfx/mod-gles2 gfx/mod-soft gfx/shared-sdl gfx/mod-caca snd/mod-csound snd/mod-ogg cli/mod-tcp cli/mod-udp cli/mod-http srv/mod-tcpd srv/mod-udpd srv/mod-httpd ; do
    ...
    echo "\tif test -w \$@ || test ! -f \$@ ; then echo doxygen $j $k && echo > \$@; for l in `echo doxygen/xml/struct${k}__* doxygen/xml/union${k}__* | sort` ; do if test -f \"\$\$l\"; then xsltproc \$(srcdir)/doxygen/texinfo-struct.xsl \"\$\$l\" | grep -v \"<?xml\" >> \$@; fi; done; fi" >> Makefile.doxygen
    echo >> Makefile.doxygen
