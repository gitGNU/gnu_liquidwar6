...
echo "\$(top_srcdir)/doc/liquidwar6-doxygen.texi: "`ls ../src/lib/*.h* | sed "s/\.\./\$\(top_srcdir\)/g" | sort` >> Makefile.doxygen
echo "\tif test -w \$@ || test ! -f \$@ ; then echo doxygen liquidwar6 && echo > \$@; for l in `echo doxygen/xml/structlw6__* | sort` ; do if test -f \"\$\$l\"; then xsltproc \$(srcdir)/doxygen/texinfo-struct.xsl \"\$\$l\" | grep -v \"<?xml\" >> \$@; fi; done; fi" >> Makefile.doxygen 
echo >> Makefile.doxygen
...
for i in bot/mod-brute bot/mod-follow bot/mod-idiot bot/mod-random gfx/mod-gl1 gfx/mod-gles2 snd/mod-csound snd/mod-ogg cli/mod-tcp cli/mod-udp cli/mod-http srv/mod-tcpd srv/mod-udpd srv/mod-httpd ; do
    j=`echo $i | cut -d "/" -f 2`
    k=`echo $j | sed "s/\-/__/g"`
    echo "\$(top_srcdir)/doc/$j-doxygen.texi: "`find ../src/lib/$i -name "*.h*" | sed "s/\.\./\$\(top_srcdir\)/g" | sort` >> Makefile.doxygen
    echo "\tif test -w \$@ || test ! -f \$@ ; then echo doxygen $j $k && echo > \$@; for l in `echo doxygen/xml/struct${k}__* | sort` ; do if test -f \"\$\$l\"; then xsltproc \$(srcdir)/doxygen/texinfo-struct.xsl \"\$\$l\" | grep -v \"<?xml\" >> \$@; fi; done; fi" >> Makefile.doxygen
    echo >> Makefile.doxygen
