...
echo "\techo doxygen liquidwar6 && rm -f \$@; for k in `echo doxygen/xml/structlw6__* | sort` ; do if test -f \"\$\$k\"; then xsltproc \$(srcdir)/doxygen/texinfo-struct.xsl \"\$\$k\" | grep -v \"<?xml\" >> \$@; fi; done" >> Makefile.doxygen 
echo >> Makefile.doxygen
...
for i in bot cfg cli cns cnx dat dsp dyn gfx glb gui hlp img ker ldr map msg net nod p2p pil scm sim snd srv sys tsk vox ; do
    ...
    echo "\techo doxygen $i && rm -f \$@; for k in `echo doxygen/xml/structlw6$i__* | sort` ; do if test -f \"\$\$k\"; then xsltproc \$(srcdir)/doxygen/texinfo-struct.xsl \"\$\$k\" | grep -v \"<?xml\" >> \$@; fi; done" >> Makefile.doxygen
    echo >> Makefile.doxygen
...
for i in bot/mod-brute bot/mod-follow bot/mod-idiot bot/mod-random gfx/mod-gl snd/mod-csound snd/mod-ogg cli/mod-tcp cli/mod-udp cli/mod-http srv/mod-tcpd srv/mod-udpd srv/mod-httpd ; do
    ...
    echo "\techo doxygen $j && rm -f \$@; for k in `echo doxygen/xml/struct$j__* | sort` ; do if test -f \"\$\$k\"; then xsltproc \$(srcdir)/doxygen/texinfo-struct.xsl \"\$\$k\" | grep -v \"<?xml\" >> \$@; fi; done" >> Makefile.doxygen
    echo >> Makefile.doxygen
