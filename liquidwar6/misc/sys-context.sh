...
l=map; for i in $(cat src/lib/$l/$l.h  | grep extern | cut -d " " -f 3 | sed s/*//g | grep lw6$l) ; do echo $l/$i; sed -i "s/\($i\) (\([^s][^y][^s][^_][^c]\)/\1 (sys_context,\2/g" src/*.c src/lib/*.c src/lib/*/*.c src/lib/*/*/*.c src/lib/*/*/*/*.c src/lib/*/*/*/*/*.c ; done
