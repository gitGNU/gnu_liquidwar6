...
static _lw6nod_test_data_t _test_data = { 0 };
...
_node_map_verified_callback (void *data)
{
  ...
  while (lw6sys_get_timestamp (sys_context,) < stop_timestamp)
    {
      lw6nod_info_map_verified_nodes (info, _node_map_verified_callback_callback, &first_time);
...
_test_node ()
{
...

  {
	...
	if (LW6SYS_TEST_ACK (lw6nod_info_add_discovered_node (info, _TEST_URL)))
	  {
	 ...
	 */
	thread_map_verified = lw6sys_thread_create (_node_map_verified_callback, NULL, info);
	if (LW6SYS_TEST_ACK (thread_map_verified))
	  {
	    thread_set_verified = lw6sys_thread_create (_node_set_verified_callback, NULL, info);
	    if (LW6SYS_TEST_ACK (thread_set_verified))
	      {
		thread_pop_discovered = lw6sys_thread_create (_node_pop_discovered_callback, NULL, info);
		if (LW6SYS_TEST_ACK (thread_pop_discovered))
		  {
		    thread_add_discovered = lw6sys_thread_create (_node_add_discovered_callback, NULL, info);
		    if (LW6SYS_TEST_ACK (thread_add_discovered))
		      {

			thread_update = lw6sys_thread_create (_node_update_callback, NULL, info);
			if (LW6SYS_TEST_ACK (thread_update))
			  {

			    thread_dup_dyn = lw6sys_thread_create (_node_dup_dyn_callback, NULL, info);
...
_test_community ()
{
...

  {
	...
	if (LW6SYS_TEST_ACK (!lw6nod_info_community_add (info, _TEST_ID_1, _TEST_URL_1)))
	  {
	...
	if (LW6SYS_TEST_ACK (lw6nod_info_community_add (info, _TEST_ID_2, _TEST_URL_2)))
	  {
	...
	if (LW6SYS_TEST_ACK (lw6nod_info_community_has_id (info, _TEST_ID_1)
			     && lw6nod_info_community_has_id (info, _TEST_ID_2) && !lw6nod_info_community_has_id (info, _TEST_ID_3)))
	  {
	...
	if (LW6SYS_TEST_ACK (lw6nod_info_community_has_url (info, _TEST_URL_1)
			     && lw6nod_info_community_has_url (info, _TEST_URL_2) && !lw6nod_info_community_has_url (info, _TEST_URL_3)))
	  {
	...
	if (LW6SYS_TEST_ACK
	    (lw6nod_info_community_is_member (info, _TEST_ID_2, _TEST_URL_3)
	     && lw6nod_info_community_is_member (info, _TEST_ID_3,
						 _TEST_URL_2)
	     && lw6nod_info_community_is_member (info, _TEST_ID_1, _TEST_URL_1) && !lw6nod_info_community_is_member (info, _TEST_ID_3, _TEST_URL_3)))
	  {
	...
	id_from_url = lw6nod_info_community_get_id_from_url (info, _TEST_URL_3);
	...
	url_from_id = lw6nod_info_community_get_url_from_id (info, _TEST_ID_3);
	...
	if (LW6SYS_TEST_ACK (peer_id_list_str))
	  {
	    if (LW6SYS_TEST_ACK (lw6sys_str_is_same (_TEST_PEER_ID_LIST_STR, peer_id_list_str)))
	      {
	...
	if (LW6SYS_TEST_ACK (lw6nod_info_community_remove_by_id (info, _TEST_ID_2)))
	  {
	...
	if (LW6SYS_TEST_ACK (lw6nod_info_community_remove_by_url (info, _TEST_URL_3)))
	  {
	    lw6sys_log (sys_context, LW6SYS_LOG_NOTICE, _x_ ("could remove peer, that's fine"));
	  }
	else
	...
	if (LW6SYS_TEST_ACK (!lw6nod_info_community_remove_by_url (info, _TEST_URL_1)))
	  {
	 ...
	 */
	lw6nod_info_community_set_peer_id_list_str (info, _TEST_PEER_ID_LIST_STR);
	lw6nod_info_community_id_without_url_map (info, _community_id_without_url_callback, (void *) &nb_ids_without_url);
	...
	if (LW6SYS_TEST_ACK (lw6nod_info_community_has_id_without_url (info, _TEST_ID_2)))
	  {
