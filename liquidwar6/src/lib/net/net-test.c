...
static _lw6net_test_data_t _test_data = { 0 };
...
_test_address ()
{
...

  {
    ...
    ret = 0;
    if (_lw6net_inet_aton (&in, _TEST_HOST_IP))
      {
...
_setup_init ()
{
  ...
  lw6sys_log (LW6SYS_LOG_NOTICE, _x_ ("init libnet CUnit test suite"));
...
_setup_quit ()
{
  ...
  lw6sys_log (LW6SYS_LOG_NOTICE, _x_ ("quit libnet CUnit test suite"));
