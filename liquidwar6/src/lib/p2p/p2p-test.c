	  ...
	  if (tcp_accepter)
	    {
	      ...
	      lw6sys_list_push_front (sys_context, &(listener->tcp_accepters),
				      tcp_accepter);
...
{
  lw6sys_log (sys_context, LW6SYS_LOG_NOTICE,
	      _x_ ("tentacle1 received \"%s\""), message);
}
...
{
  lw6sys_log (sys_context, LW6SYS_LOG_NOTICE,
	      _x_ ("tentacle2 received \"%s\""), message);
}
...
_test_tentacle1_thread_callback (void *tentacle_data)
{
  ...
  end_timestamp =
    lw6sys_get_timestamp (sys_context,) + _TEST_TENTACLE_DURATION_THREAD;
  ...
  if (lw6cnx_ticket_table_init
      (&ticket_table, _TEST_TENTACLE_TICKET_TABLE_HASH_SIZE))
    {
	  ...
	  (&(data->tentacle), &(data->backends), data->listener,
	   ...
	   tentacle_data))
	{
	  while ((now = lw6sys_get_timestamp (sys_context,)) < end_timestamp
		 && !(*(data->done)))
	    {
	      lw6sys_idle ();
	      logical_ticket_sig =
    ...
    {
      lw6sys_log (sys_context, LW6SYS_LOG_WARNING,
		  _x_ ("unable to init ticket table 1"));
      data->ret = 0;
    }
}
  ...
  if (lw6cnx_ticket_table_init
	   ...
	   tentacle_data))
	{
	  while ((now = lw6sys_get_timestamp (sys_context,)) < end_timestamp
		...
		lw6msg_ticket_calc_sig (lw6cnx_ticket_table_get_send
					(&ticket_table,
					 _TEST_TENTACLE_ID1_STR),
					_TEST_TENTACLE_ID2,
					_TEST_TENTACLE_ID1,
					_TEST_TENTACLE_SEND_BEST_UNRELIABLE_STR);
	      if (_lw6p2p_tentacle_send_best
		  (&(data->tentacle), lw6sys_get_timestamp (sys_context,),
		   &ticket_table, logical_ticket_sig, _TEST_TENTACLE_ID2,
		   _TEST_TENTACLE_ID1,
		   _TEST_TENTACLE_SEND_BEST_UNRELIABLE_STR, 0))
		{
		  nb_sent_best_unreliable++;
    ...
    {
      lw6sys_log (sys_context, LW6SYS_LOG_WARNING,
		  _x_ ("unable to init ticket table 2"));
      data->ret = 0;
    }
}
...
_tentacle_with_backends (char *cli_backends, char *srv_backends)
{
  ...
  if (ret)
	...
	lw6sys_thread_create (_test_tentacle2_thread_callback, NULL,
      ...
      lw6sys_progress_default (sys_context, &(tentacle_data1.progress),
			       &(tentacle_data1.progress_value));
      lw6sys_progress_default (sys_context, &(tentacle_data2.progress),
...

  {
    ...
    packet_1 =
      _lw6p2p_packet_new (_TEST_PACKET_LOGICAL_TICKET_SIG_1,
			  _TEST_PACKET_PHYSICAL_TICKET_SIG_1,
			  _TEST_PACKET_LOGICAL_FROM_ID_1,
			  _TEST_PACKET_LOGICAL_TO_ID_1, _TEST_PACKET_MSG_1);
    if (LW6SYS_TEST_ACK (packet_1))
      {
	packet_2 =
	  _lw6p2p_packet_new (_TEST_PACKET_LOGICAL_TICKET_SIG_2,
			      _TEST_PACKET_PHYSICAL_TICKET_SIG_2,
			      _TEST_PACKET_LOGICAL_FROM_ID_2,
			      _TEST_PACKET_LOGICAL_TO_ID_2,
			      _TEST_PACKET_MSG_2);
	if (LW6SYS_TEST_ACK (packet_2))
	  {
	    ...
	    if (LW6SYS_TEST_ACK
		(checksum_1 == _TEST_PACKET_CHECKSUM_1
		 && checksum_2 == _TEST_PACKET_CHECKSUM_2))
	      {
		lw6sys_log (sys_context, LW6SYS_LOG_NOTICE,
			    _x_ ("packet checksums OK (%08x,%08x)"),
		...
		lw6sys_log (sys_context, LW6SYS_LOG_WARNING,
			    ...
			    checksum_1, checksum_2, _TEST_PACKET_CHECKSUM_1,
			    _TEST_PACKET_CHECKSUM_2);
		ret = 0;
	      }
	    if (LW6SYS_TEST_ACK
		(_lw6p2p_packet_compare (packet_1, packet_2) ==
		 _TEST_PACKET_COMPARE))
	      {
		lw6sys_log (sys_context, LW6SYS_LOG_NOTICE,
			    _x_ ("packet compare works as expected"));
	      }
	    else
	      {
		lw6sys_log (sys_context, LW6SYS_LOG_WARNING,
			    _x_ ("packet compare does not work as expected"));
		ret = 0;
	      }
	     ...
	     lw6p2p_node_t * node5, lw6p2p_node_t * node6)
{
  ...
  next_timestamp =
    lw6sys_get_timestamp (sys_context,) + _TEST_NODE_POLL_DURATION;
...
_cmd_with_backends (char *cli_backends, char *srv_backends)
{
  ...
  ret = 0;
  end_timestamp =
    lw6sys_get_timestamp (sys_context,) + _TEST_NODE_CMD_DURATION;
  if (_init_nodes
							   ...
							   (node1))))
	{
	  ...
	  if (entries)
	    {
	      lw6sys_list_map (sys_context, entries, _print_entry_callback,
			       (void *) node1);
...
_oob_with_backends (char *cli_backends, char *srv_backends)
{
  ...
  ret = 0;
  refresh_timestamp =
    lw6sys_get_timestamp (sys_context,) + _TEST_NODE_REFRESH_DURATION;
  end_timestamp =
    lw6sys_get_timestamp (sys_context,) + _TEST_NODE_OOB_DURATION;
  if (_init_nodes
      (cli_backends, srv_backends, &db12, &db34, &db56, &node1, &node2,
       &node3, &node4, &node5, &node6))
    {
	...
	if (LW6SYS_TEST_ACK (node))
	  {
		...
		for (j = 0; j < _TEST_NODE_MSG_NB_PER_SEQ; ++j, ++k)
		  {
		    random_str =
		      lw6sys_str_random_words (sys_context, lw6sys_random
			...
			if (LW6SYS_TEST_ACK (msg))
			  {
			    checksums[k] =
			      lw6sys_checksum_str (sys_context, msg);
			    msgs[k] = msg;
		...
		for (i = 0;
		     ...
		     ++i)
		  {
			...
			&& lw6sys_str_is_same (sys_context, msg, msgs[i])
			&& !found)
		      {
			found = 1;
		      }
	    ...
	    if (LW6SYS_TEST_ACK
		(k == _TEST_NODE_MSG_NB_SEQS * _TEST_NODE_MSG_NB_PER_SEQ))
	      {
		lw6sys_log (sys_context, LW6SYS_LOG_NOTICE,
			    _x_ ("got %d draft messages"), k);
	      }
	    else
	      {
		lw6sys_log (sys_context, LW6SYS_LOG_WARNING,
...
_test_node_api_node1_callback (void *api_data)
{
  ...
  end_timestamp =
    lw6sys_get_timestamp (sys_context,) + _TEST_NODE_API_DURATION_THREAD;
...
_test_node_api_node2_callback (void *api_data)
{
  ...
  begin_timestamp =
    lw6sys_get_timestamp (sys_context,) + _TEST_NODE_ACK_DELAY_MSEC;
  ...
  if (data->ret)
    {
      lw6sys_log (sys_context, LW6SYS_LOG_NOTICE, _x_ ("\"%s\" is ready"),
		  ((_lw6p2p_node_t *) data->node)->public_url);
    }
  else
    {
      lw6sys_log (sys_context, LW6SYS_LOG_WARNING,
		  _x_ ("could not start \"%s\""),
		  ((_lw6p2p_node_t *) data->node)->public_url);
    }
  ...
  while (lw6sys_get_timestamp (sys_context,) < end_timestamp
	...
	{
	  dump =
	    lw6sys_str_random_words (sys_context, _TEST_NODE_API_DUMP_SIZE);
	  if (dump)
	    {
	      msg =
	      ...
	      if (msg)
		{
	       ...
	       */
	      tmp =
		lw6sys_str_random_word (sys_context,
					_TEST_NODE_API_FINAL_SIZE);
	      if (tmp)
		{
		  msg =
		    lw6sys_new_sprintf (sys_context,
      ...
      if (dump_sent)
	{
	  if (next_msgn_timestamp < lw6sys_get_timestamp ())
		...
		_TEST_NODE_API_MSGN_DELAY_MSEC;
	    }
	}
    }
...
_test_node_api_node4_callback (void *api_data)
{
  ...
  if (LW6SYS_TEST_ACK (lw6p2p_node_client_join
...
_api_with_backends (char *cli_backends, char *srv_backends)
{
  ...
  if (LW6SYS_TEST_ACK (_init_nodes
			...
			&node1, &node2, &node3, &node4, &node5, &node6)))
    {
      ...
      api_data5.done = &done;
      api_data6.done = &done;
      lw6sys_progress_default (sys_context, &(api_data1.progress),
			       &(api_data1.progress_value));
      lw6sys_progress_default (sys_context, &(api_data2.progress),
			       &(api_data2.progress_value));
      lw6sys_progress_default (sys_context, &(api_data3.progress),
			       &(api_data3.progress_value));
      lw6sys_progress_default (sys_context, &(api_data4.progress),
			       &(api_data4.progress_value));
      lw6sys_progress_default (sys_context, &(api_data5.progress),
			       &(api_data5.progress_value));
      lw6sys_progress_default (sys_context, &(api_data6.progress),
      ...
      if (thread6)
	{
      ...
      end_timestamp =
	lw6sys_get_timestamp (sys_context,) + _TEST_NODE_API_DURATION_END;
      while (lw6sys_get_timestamp (sys_context,) < end_timestamp)
	{
	  _poll_nodes (node1, node2, node3, node4, node5, node6);
	}
...
_setup_init ()
{
  ...
  lw6sys_log (sys_context, LW6SYS_LOG_NOTICE,
	      _x_ ("init libp2p CUnit test suite"));
...
_setup_quit ()
{
  ...
  lw6sys_log (sys_context, LW6SYS_LOG_NOTICE,
	      _x_ ("quit libp2p CUnit test suite"));
