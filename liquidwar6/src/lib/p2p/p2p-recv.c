		      ...
		      char *message)
{
  ...
  if (node && cnx)
    {
      recv_ticket=lw6cnx_ticket_table_get_recv(&(node->ticket_table),cnx->remote_id_str);
      if ((!lw6cnx_ticket_table_was_recv_exchanged(&node->ticket_table,cnx->remote_id_str)) || lw6msg_ticket_check_sig(recv_ticket,cnx->remote_id_int,cnx->local_id_int,message,physical_ticket_sig)) {
	if (recv_ticket) {
	...
	if (cnx->local_id_int==logical_to_id) {
	  if( logical_from_id==cnx->remote_id_int) {
	    ...
	    if (logical_from_id_str) {
	      recv_ticket=lw6cnx_ticket_table_get_recv(&(node->ticket_table),logical_from_id_str);
	      if ((!lw6cnx_ticket_table_was_recv_exchanged(&node->ticket_table,logical_from_id_str)) || lw6msg_ticket_check_sig(recv_ticket,logical_from_id,logical_to_id,message,logical_ticket_sig)) {
		if (recv_ticket) {
	  ...
	  if (logical_sig_ok) {
	    ...
	    lw6sys_log(LW6SYS_LOG_NOTICE,_("bad logical ticket_sig from \"%s\""),cnx->remote_url);
	  }
	} else {
	  if (cnx->remote_id_int==logical_from_id) {
	    _lw6p2p_recv_forward(node,cnx,logical_ticket_sig,logical_from_id,logical_to_id,message);
	  } else {
	    lw6sys_log(LW6SYS_LOG_WARNING,_("open relay not allowed (yet)"));
	  }
	}
      } else {
    lw6sys_log(LW6SYS_LOG_NOTICE,_("bad physical ticket_sig from \"%s\""),cnx->remote_url);
  }
}
}
