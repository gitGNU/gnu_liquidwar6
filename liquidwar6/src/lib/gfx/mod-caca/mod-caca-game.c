		       ...
		       lw6pil_local_cursors_t * local_cursors)
{
  ...
  if (buffer == NULL)
    return 0;
  memset (buffer, lw6sys_color_8_to_i (caca_context->const_data.bg_color),
  ...
  for (y = 0; y < height; y++)
      ...
      for (x = 0; x < width; x++)
	{
	  ...
	  if (fighter_id >= 0)
		...
		{
		  buffer[width * y + x] =
		    lw6sys_color_8_to_i (caca_context->
					 const_data.team_color[team_color]);
		}
	    }
	  else
	    {
		...
		buffer[width * y + x] =
		  lw6sys_color_8_to_i (caca_context->const_data.bg_color);
	      else
		buffer[width * y + x] =
		  lw6sys_color_8_to_i (caca_context->const_data.fg_color);
	    }
	}
    }
  ...
  caca_dither_bitmap (caca_context->canvas, 0, 0,
		      caca_get_canvas_width (caca_context->canvas),
		      caca_get_canvas_height (caca_context->canvas), dither,
		      buffer);
