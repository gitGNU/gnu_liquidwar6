...
static int
mod_caca_score_font (_mod_caca_context_t * caca_context, int player_id,
		     lw6ker_score_t * score)
{
  int wc, hc, i;
  caca_canvas_t *cv;
  caca_font_t *f;
  ...
  if (player_id < 5)
    caca_dither_bitmap (caca_context->canvas, (wc / 2) - 4,
			(hc / 5) * player_id, 4, (hc / 5), d, buffer);
  else
    caca_dither_bitmap (caca_context->canvas, wc - 4,
			(hc / 5) * (player_id - 5), 4, (hc / 5), d, buffer);
  ...
  d = caca_create_dither (32, wc, hc, 4 * wc,
			  0xff00, 0xff0000, 0xff000000, 0xff);

  if (player_id < 5)
    caca_dither_bitmap (caca_context->canvas, 0, (hc / 5) * player_id,
			wc, 22, d, buf);
  else
    caca_dither_bitmap (caca_context->canvas, (wc / 2),
			(hc / 5) * (player_id - 5), wc, 22, d, buf);
...
static int
mod_caca_score_text (_mod_caca_context_t * caca_context, int player_id,
		     lw6ker_score_t * score)
{
  // int wc, hc, i;
  int i;
  ...
  memset (score_str, 0, 32);
  snprintf (score_str, 12, " P%2d: %d %% ", player_id,
	    score->fighters_percent);
  ...
  dither = caca_create_dither (32, 3, 1, 4 * 3,
			       0x00ff0000, 0x0000ff00, 0x000000ff, 0x0);
  caca_dither_bitmap (caca_context->canvas, player_id * 15, 0,
		      3, 1, dither, buffer);
  ...
  for (i = 0; i < LW6MAP_NB_TEAM_COLORS; ++i)
    if (caca_context->score_array.scores[i].has_been_active)
      mod_caca_score_text (caca_context, i,
			   &caca_context->score_array.scores[i]);
  caca_refresh_display (caca_context->display);
  return ret;
}
