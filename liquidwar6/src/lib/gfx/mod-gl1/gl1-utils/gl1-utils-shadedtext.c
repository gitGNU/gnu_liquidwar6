  ...
  if (change && shaded_text->text)
    {
      if (shaded_text->bg)
	{
	  mod_gl1_utils_bitmap_free (utils_context, shaded_text->bg);
	}
      if (shaded_text->fg)
	{
      ...
      if (surface_bg)
	{
	  ...
	  if (shaded_text->bg)
	    ...
	    {
	      mod_gl1_utils_delete_surface (utils_context, surface_bg);
	    }
	}
      if (surface_fg)
	{
	  ...
	  if (shaded_text->fg)
	    ...
	    {
	      mod_gl1_utils_delete_surface (utils_context, surface_fg);
	    }
	}
