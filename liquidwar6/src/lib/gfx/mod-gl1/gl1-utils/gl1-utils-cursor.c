				   ...
				   lw6map_level_t * level)
{
  ...
  surface =
    mod_gl1_utils_create_surface (utils_context, LW6MAP_CURSOR_TEXTURE_SIZE,
  ...
  if (surface)
      ...
      for (y = 0; y < LW6MAP_CURSOR_TEXTURE_SIZE; ++y)
	{
      ...
      bitmap =
	mod_gl1_utils_surface2bitmap (utils_context, surface,
				      _DESC_CURSOR_FG_BG);
      if (bitmap)
	{
	  mod_gl1_utils_bitmap_colorize (utils_context, bitmap,
				       ...
				       lw6ker_cursor_t * cursor)
{
  ...
  if (surface)
      ...
      if (bitmap)
	{
	  ...
	  mod_gl1_utils_bitmap_colorize (utils_context, bitmap, 1,
					 &color_couple);
	}
      else
	{
