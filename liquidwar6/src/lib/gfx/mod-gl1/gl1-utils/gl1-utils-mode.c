 ...
 */
int
mod_gl1_utils_set_video_mode (mod_gl1_utils_context_t * utils_context,
			      lw6gui_video_mode_t * video_mode)
{
  ...
  lw6sys_sleep (sys_context,
		utils_context->sdl_context.const_data.mode_sleep);
  ...
  if (ok)
	...
	default:
      ...
      if (width <= 0 || height <= 0)
	{
	  if (mod_gl1_utils_get_fullscreen_modes
      ...
      if (fullscreen)
	{
	  if (mod_gl1_utils_get_fullscreen_modes
	      ...
	      if (lw6gui_video_mode_sync_ratio (&target_mode, &ratio_mode))
		{
		  ...
		  mod_gl1_utils_find_closest_resolution (utils_context,
							 &width_test,
							 &height_test,
							 width_test,
							 height_test);
      ...
      if (fullscreen)
	{
      ...
      if (utils_context->caps.max_texture_size <
	  MOD_GL1_UTILS_REQUIRED_TEXTURE_SIZE)
	{
	  lw6sys_log (sys_context, LW6SYS_LOG_WARNING,
		      _x_
 ...
 */
int
mod_gl1_utils_resize_video_mode (mod_gl1_utils_context_t * utils_context,
				 lw6gui_video_mode_t * video_mode)
{
  int ret = 0;
...
int
mod_gl1_utils_get_video_mode (mod_gl1_utils_context_t * utils_context,
			      lw6gui_video_mode_t * video_mode)
{
  int ret = 0;
...
int
mod_gl1_utils_set_resize_callback (mod_gl1_utils_context_t * utils_context,
...
mod_gl1_utils_call_resize_callback (mod_gl1_utils_context_t * utils_context)
{
  if (utils_context->sdl_context.resize_callback)
	...
	sdl_context.resize_callback (&
				     (utils_context->sdl_context.video_mode));
    }
}
