...
_warning (const char *func_name)
{
  lw6sys_log (LW6SYS_LOG_WARNING,
	      _("background backend function \"%s\" is not defined"));
}

int
mod_gl_background_init (mod_gl_utils_context_t * utils_context,
...
void
mod_gl_background_quit (mod_gl_utils_context_t * utils_context,
			mod_gl_background_backend_t * backend)
{
  if (backend->quit)
    {
...
void
mod_gl_background_display_background (mod_gl_utils_context_t * utils_context,
				      mod_gl_background_backend_t * backend,
				      lw6gui_look_t * look)
{
