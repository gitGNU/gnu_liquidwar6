				   ...
				   lw6sys_color_8_t team_color, int j)
{
  ...
  color =
    lw6sys_color_ponderate (dead_color, team_color,
			    ((float) j) /
			    ((float) MOD_GL_SHADES_FOR_FIGHTERS_SCALE));
  color.a = 0xFF;
...
int
mod_gl_utils_team_color_map_is_same (mod_gl_utils_team_color_map_t
				     * team_color_map_a,
				     ...
				     team_color_map_b)
{
  ...
  ret =
    !memcmp (team_color_map_a, team_color_map_b,
	     sizeof (mod_gl_utils_team_color_map_t));

  return ret;
}

void
mod_gl_utils_force_color32_alpha (Uint32 * color, float alpha)
{
					     ...
					     float health)
{
  ...
  f = health * (float) MOD_GL_SHADES_FOR_FIGHTERS_SCALE;
