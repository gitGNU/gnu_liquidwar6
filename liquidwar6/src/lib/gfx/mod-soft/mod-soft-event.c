...
key_down (lw6gui_keyboard_t * keyboard, SDL_Event * event,
	  _mod_soft_const_data_t * const_data, int64_t timestamp)
{
  int sym = 0;
...
static int
joystick_index (SDL_Event * event, _mod_soft_const_data_t * const_data)
{
  int ret = -1;
...
joystick_button_index (SDL_Event * event,
		       _mod_soft_const_data_t * const_data, int i)
{
  int ret = -1;
 ...
 */
lw6gui_input_t *
_mod_soft_pump_events (_mod_soft_context_t * utils_context)
{
  lw6gui_video_mode_t video_mode;
  SDL_Event event;
  int64_t timestamp;
  lw6gui_input_t *input = &(utils_context->input);
  _mod_soft_const_data_t *const_data = &(utils_context->const_data);
  int i, b;

  timestamp = _mod_soft_timer_get_timestamp (utils_context);
  ...
  while (SDL_PollEvent (&event))
	...
	case SDL_JOYBUTTONUP:
	  ...
	  break;
	case SDL_VIDEORESIZE:
	  _mod_soft_get_video_mode (utils_context, &video_mode);
	  video_mode.width = event.resize.w;
	  video_mode.height = event.resize.h;
	  _mod_soft_resize_video_mode (utils_context, &video_mode);
	  //_mod_soft_timer_set_bitmap_refresh (utils_context);
	  break;
	case SDL_VIDEOEXPOSE:
	  _mod_soft_sync_mode (utils_context, 0);
	  //_mod_soft_timer_set_bitmap_refresh (utils_context);
	  break;
	}
    }
