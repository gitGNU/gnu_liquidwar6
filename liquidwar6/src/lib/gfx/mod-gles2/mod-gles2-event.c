...
key_down (lw6gui_keyboard_t * keyboard, SDL_Event * event,
	  _mod_gles2_const_data_t * const_data, int64_t timestamp)
{
  int sym = 0;
		   ...
		   int64_t timestamp)
{
  switch (event->button.button)
    ...
    case SDL_BUTTON_MIDDLE:
      lw6gui_button_register_down (&(mouse->button_middle), timestamp);
      break;
...
mouse_button_up (lw6gui_mouse_t * mouse, SDL_Event * event)
{
  switch (event->button.button)
    ...
    case SDL_BUTTON_RIGHT:
      ...
      break;
    case SDL_BUTTON_MIDDLE:
      lw6gui_button_register_up (&(mouse->button_middle));
      break;
    case SDL_BUTTON_WHEELUP:
      lw6gui_button_register_up (&(mouse->wheel_up));
      break;
...
static int
joystick_index (SDL_Event * event, _mod_gles2_const_data_t * const_data)
{
  int ret = -1;
...
joystick_button_index (SDL_Event * event,
		       _mod_gles2_const_data_t * const_data, int i)
{
  int ret = -1;
		      ...
		      int64_t timestamp)
{
  switch (b)
    ...
    case LW6GUI_JOYSTICK_BUTTON_ID_E:
      lw6gui_button_register_down (&(joystick->button_e), timestamp);
      break;
...
joystick_button_up (lw6gui_joystick_t * joystick, int b, SDL_Event * event)
{
  switch (b)
    ...
    case LW6GUI_JOYSTICK_BUTTON_ID_D:
      ...
      break;
    case LW6GUI_JOYSTICK_BUTTON_ID_E:
      lw6gui_button_register_up (&(joystick->button_e));
      break;
    case LW6GUI_JOYSTICK_BUTTON_ID_F:
 ...
 */
lw6gui_input_t *
_mod_gles2_pump_events (_mod_gles2_context_t * utils_context)
{
  lw6gui_video_mode_t video_mode;
  SDL_Event event;
  int64_t timestamp;
  lw6gui_input_t *input = &(utils_context->input);
  _mod_gles2_const_data_t *const_data = &(utils_context->const_data);
  int i, b;

  timestamp = _mod_gles2_timer_get_timestamp (utils_context);
  ...
  while (SDL_PollEvent (&event))
	...
	case SDL_JOYBUTTONUP:
	  ...
	  break;
	case SDL_VIDEORESIZE:
	  _mod_gles2_get_video_mode (utils_context, &video_mode);
	  video_mode.width = event.resize.w;
	  video_mode.height = event.resize.h;
	  _mod_gles2_resize_video_mode (utils_context, &video_mode);
	  //_mod_gles2_timer_set_bitmap_refresh (utils_context);
	  break;
	case SDL_VIDEOEXPOSE:
	  _mod_gles2_sync_mode (utils_context, 0);
	  //_mod_gles2_timer_set_bitmap_refresh (utils_context);
	  break;
	}
    }
