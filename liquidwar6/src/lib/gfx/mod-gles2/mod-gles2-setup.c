...
_mod_gles2_init (lw6sys_context_t * sys_context, int argc, const char *argv[], lw6gui_video_mode_t * video_mode, lw6gui_resize_callback_func_t resize_callback)
{
  ...
  if (gles2_context)
    {
      gles2_context->shared_sdl_handle = lw6dyn_dlopen_shared (sys_context, argc, argv, "gfx", "sdl");
      if (gles2_context->shared_sdl_handle == NULL)
	{
	  lw6sys_log (sys_context, LW6SYS_LOG_WARNING, _x_ ("unable to load shared SDL code"));
	  _mod_gles2_quit (sys_context, gles2_context);
	  gles2_context = NULL;
	}
    }
#endif // LW6_ALLINONE
  ...
  if (gles2_context)
    {
      _lw6gfx_sdl_bind_funcs (sys_context, &(gles2_context->sdl_context.funcs), gles2_context->shared_sdl_handle);

      if (_mod_gles2_path_init (sys_context, &(gles2_context->path), argc, argv)
	  && _lw6gfx_sdl_path_init (sys_context, &(gles2_context->sdl_context), argc, argv))
	{
	  ...
	  if (gles2_context)
	      ...
	      if (gles2_context && sdl_ok && ttf_ok)
		{
		  ...
		  _mod_gles2_set_resize_callback (sys_context, gles2_context, resize_callback);
		  if (_mod_gles2_load_consts (sys_context, gles2_context) && _lw6gfx_sdl_load_consts (sys_context, &(gles2_context->sdl_context)))
		    {
		      if (_mod_gles2_set_video_mode (sys_context, gles2_context, video_mode))
			{
			  _lw6gfx_sdl_timer_update (sys_context, &(gles2_context->sdl_context));
			  // todo
			}
		      else
			{
...
_mod_gles2_quit (lw6sys_context_t * sys_context, _mod_gles2_context_t * gles2_context)
{
  ...
  _lw6gfx_sdl_unload_consts (sys_context, &(gles2_context->sdl_context));
  _mod_gles2_unload_consts (sys_context, gles2_context);

  _lw6gfx_sdl_path_quit (sys_context, &(gles2_context->sdl_context));
  _mod_gles2_path_quit (sys_context, &(gles2_context->path));

  _lw6gfx_sdl_unbind_funcs (sys_context, &(gles2_context->sdl_context.funcs));

#ifndef LW6_ALLINONE
  if (gles2_context->shared_sdl_handle)
    {
      lw6dyn_dlclose_shared (sys_context, gles2_context->shared_sdl_handle);
