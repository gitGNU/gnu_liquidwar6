...
_mod_gles2_init (int argc, const char *argv[], lw6gui_video_mode_t * video_mode, lw6gui_resize_callback_func_t resize_callback)
{
  ...
  if (gles2_context)
    {
      _lw6gfx_sdl_bind_funcs (&(gles2_context->sdl_context.funcs), gles2_context->shared_sdl_handle);

      if (_mod_gles2_path_init (&(gles2_context->path), argc, argv) && _lw6gfx_sdl_path_init (&(gles2_context->sdl_context), argc, argv))
	{
	  ...
	  if (gles2_context)
	      ...
	      if (gles2_context && sdl_ok && ttf_ok)
		{
		  ...
		  if (_mod_gles2_load_consts (gles2_context) && _lw6gfx_sdl_load_consts (&(gles2_context->sdl_context)))
		    {
		      if (_mod_gles2_set_video_mode (gles2_context, video_mode))
			{
			  _lw6gfx_sdl_timer_update (&(gles2_context->sdl_context));
			  // todo
			}
		      else
			{
