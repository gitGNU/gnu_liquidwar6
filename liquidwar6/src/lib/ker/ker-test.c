...
_print_game_struct_repr (const lw6ker_game_struct_t * game_struct)
{
  ...
  x =
    lw6sys_random (sys_context,
		   lw6ker_game_struct_get_w (sys_context, game_struct));
  y =
    lw6sys_random (sys_context,
		   lw6ker_game_struct_get_h (sys_context, game_struct));
  z =
    lw6sys_random (sys_context,
		   lw6ker_game_struct_get_d (sys_context, game_struct));
  ...
  zone_id =
    lw6ker_game_struct_get_zone_id (sys_context, game_struct, shape.w / 2,
				    shape.h / 2, shape.d / 2);
  lw6ker_game_struct_get_zone_info (sys_context, game_struct, zone_id,
				    &zone_pos, &zone_size);
  lw6sys_log (sys_context, LW6SYS_LOG_NOTICE,
	      _x_ ("zone_id=%d zone_pos=%d,%d,%d zone_size=%d"), zone_id,
	      zone_pos.x, zone_pos.y, zone_pos.z, zone_size);
  lw6ker_game_struct_find_free_slot_near (sys_context, game_struct, &there,
					  here);
...
_print_game_state_repr (const lw6ker_game_state_t * game_state)
{
  ...
  total_fighters =
    lw6ker_game_state_get_nb_active_fighters (sys_context, game_state);
  ...
  if (total_fighters > 0)
	...
	{
	  lw6ker_game_state_get_team_info (sys_context, game_state, i,
	  ...
	  x =
	    lw6sys_random (sys_context,
			   lw6ker_game_state_get_w (sys_context, game_state));
	  y =
	    lw6sys_random (sys_context,
			   lw6ker_game_state_get_h (sys_context, game_state));
	  z =
	    lw6sys_random (sys_context,
			   lw6ker_game_state_get_d (sys_context, game_state));
	  potential =
	  ...
	  winner =
	    lw6ker_game_state_get_winner (sys_context, game_state,
					  LW6MAP_TEAM_COLOR_INVALID);
	  looser =
	    lw6ker_game_state_get_looser (sys_context, game_state,
					  LW6MAP_TEAM_COLOR_INVALID);
	  lw6sys_log (sys_context, LW6SYS_LOG_NOTICE,
		      _x_ ("winner=%d looser=%d"), winner, looser);
	  lw6ker_move_get_best_next_pos (sys_context, game_state, &next_pos,
					 &current_pos, 0);
	  lw6ker_game_state_get_charge_per1000 (sys_context, game_state, 0);
	  lw6ker_game_state_get_weapon_per1000_left (sys_context, game_state,
						     0);
	  lw6ker_game_state_get_latest_weapon (sys_context, game_state,
					       &team_color, &weapon_id,
					       &per1000_left);
	}
    }
  ...
  lw6sys_log (sys_context, LW6SYS_LOG_NOTICE,
	      ...
	      lw6ker_game_state_get_nb_cursors (sys_context, game_state),
	      lw6ker_game_state_get_nb_nodes (sys_context, game_state));
  lw6sys_log (sys_context, LW6SYS_LOG_NOTICE,
	      _x_ ("time_elapsed=%d time_left=%d"),
	      lw6ker_game_state_get_time_elapsed (sys_context, game_state),
	      lw6ker_game_state_get_time_left (sys_context, game_state));
}
	...
	if (LW6SYS_TEST_ACK (game_struct))
	  {
		...
		cursor.pos.y =
		  lw6ker_game_state_get_h (sys_context, game_state) / 2;
		...
		lw6ker_game_state_set_cursor (sys_context, game_state,
					      &cursor);
		lw6ker_game_state_get_cursor (sys_context, game_state,
		...
		lw6ker_score_array_update (sys_context, &score_array,
					   game_state);
	...
	if (LW6SYS_TEST_ACK (game_struct))
	  {
	    ...
	    hexa_struct =
	      lw6ker_game_struct_to_hexa (sys_context, game_struct);
	    lw6ker_game_struct_free (sys_context, game_struct);
	    game_struct = NULL;
		...
		if (LW6SYS_TEST_ACK
		  ...
		  {
		    game_struct2 =
		      lw6ker_game_struct_from_hexa (sys_context, hexa_struct,
						    level);
			 ...
			 */
			dup_game_struct =
			  lw6ker_game_struct_dup (sys_context, game_struct2,
						  NULL);
			...
			if (LW6SYS_TEST_ACK (hexa_struct2))
			  {
				...
				(checksum_struct == checksum_struct2
				 && lw6sys_str_is_same (sys_context,
			      ...
			      {
				game_state =
				  lw6ker_game_state_new (sys_context,
							 game_struct2, NULL);
				if (LW6SYS_TEST_ACK (game_state))
				  {
