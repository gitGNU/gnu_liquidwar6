...
#else
  ret = lw6dyn_list_backends (sys_context, argc, argv, "bot");
#endif
...
lw6bot_create_backend (lw6sys_context_t * sys_context, int argc, const char *argv[], const char *name)
{
  ...
  if (backend)
	...
	{
	  lw6bot_destroy_backend (sys_context, backend);
	  backend = NULL;
	}
    }
  else
    {
      lw6sys_log (sys_context, LW6SYS_LOG_WARNING, _x_ ("bot backend \"%s\" does not exist"), name);
    }
#else
  lw6dyn_dl_handle_t *backend_handle = NULL;

  backend_handle = lw6dyn_dlopen_backend (sys_context, argc, argv, "bot", name);
