  ...
  if (ret)
      ...
      if (CU_initialize_registry () == CUE_SUCCESS)
	{
	  ret =
	    ...
	    && lw6mat_test_register (mode) && lw6gui_test_register (sys_context, mode)
	    ...
	    && lw6p2p_test_register (sys_context, mode) && lw6scm_test_register (sys_context, mode) && lw6_test_register (mode);
