	      ...
	      lw6map_metadata_t * metadata)
{
  ...
  ret = ret
    && lw6sys_hexa_serializer_pop_str (sys_context, hexa_serializer,
				       &(metadata->title));
  ret = ret
    && lw6sys_hexa_serializer_pop_str (sys_context, hexa_serializer,
				       &(metadata->author));
  ...
  if (layer->shape.w == shape->w && layer->shape.h == shape->h)
      ...
      if (layer->data)
	{
	  for (y = 0; y < layer->shape.h && ret; ++y)
		...
		{
		  ret = ret
		    && lw6sys_hexa_serializer_pop_int8 (sys_context,
							hexa_serializer, &c);
	     ...
	     lw6map_texture_t * texture)
{
  ...
  if (texture->w >= LW6MAP_MIN_TEXTURE_WIDTH
      ...
      if (texture->data)
	{
	  for (y = 0; y < texture->h && ret; ++y)
		...
		{
		  ret = ret
		    && lw6sys_hexa_serializer_pop_color (sys_context,
							 hexa_serializer,
							 &color);
  ...
  ret = ret
    && lw6sys_hexa_serializer_pop_color (sys_context, hexa_serializer,
					 &(texture->guessed_color_base.bg));
  ret = ret
    && lw6sys_hexa_serializer_pop_color (sys_context, hexa_serializer,
					 &(texture->guessed_color_base.fg));
...
pop_param (lw6sys_hexa_serializer_t * hexa_serializer, lw6map_param_t * param)
{
  ...
  ret = ret
    && lw6sys_hexa_serializer_pop_int32 (sys_context, hexa_serializer,
					 &(param->style.keep_ratio));
  ret = ret
    && lw6sys_hexa_serializer_pop_float (sys_context, hexa_serializer,
					 &(param->style.zoom));
  ret = ret
    && lw6sys_hexa_serializer_pop_float (sys_context, hexa_serializer,
					 &(param->style.zoom_min));
