	    ...
	    lw6map_layer_t * layer)
{
  ...
  ret = ret
    && lw6sys_hexa_serializer_push_whd (hexa_serializer, layer->shape);
		 ...
		 lw6map_meta_layer_t * meta_layer)
{
  ...
  ret = ret
    && lw6sys_hexa_serializer_push_whd (hexa_serializer, meta_layer->shape);
	   ...
	   lw6sys_whd_t * shape)
{
  ...
  ret = ret
    && lw6sys_hexa_serializer_pop_whd (hexa_serializer, &(layer->shape));
	     ...
	     lw6map_texture_t * texture)
{
  ...
  ret = ret
    && lw6sys_hexa_serializer_pop_int32 (hexa_serializer,
					 &(texture->has_alpha));
  if (texture->w >= LW6MAP_MIN_SIDE && texture->w <= LW6MAP_MAX_SIDE
      && texture->h >= LW6MAP_MIN_SIDE && texture->h <= LW6MAP_MAX_SIDE
