	      ...
	      u_int64_t * remote_id, char **remote_url)
{
  ...
  if (udpd_context)
    {
      ret =
	_mod_udpd_analyse_tcp (udpd_context, tcp_accepter, remote_id,
			       remote_url);
    }
	      ...
	      u_int64_t * remote_id, char **remote_url)
{
  ...
  if (udpd_context)
    {
      ret =
	_mod_udpd_analyse_udp (udpd_context, udp_buffer, remote_id,
			       remote_url);
    }
       ...
       u_int64_t logical_to_id, char *message)
{
  ...
  if (udpd_context)
    {
      ret =
	_mod_udpd_send (udpd_context, connection, ticket_sig, logical_from_id,
